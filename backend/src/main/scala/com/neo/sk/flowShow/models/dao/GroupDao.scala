...
object GroupDao {
  ...
  def addGroup(name:String, duration:Long, userId:Long, timestamp:Long, map: String) = db.run(
    tGroups.returning(tGroups.map(_.groupId)).+=(rGroups(-1l, name, userId, timestamp, duration, map))
  )
