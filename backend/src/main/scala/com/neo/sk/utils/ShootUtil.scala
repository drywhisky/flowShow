...
object ShootUtil {
  ...
  def boxLine2Shoot(line:String,id:Long,apMac:String) = {
    ...
    if (infos.length == 4) {
      ...
      val src = infos(3)
      if (rssi.length == 3) {
        val shoot = Shoot(id,apMac, clientMac, t, rssi, src)
        log.info(s"data line $line to shoot ${shoot.toString}")
        Some(shoot)
      } else {
        log.warn(s"data format of AP [$apMac] error, rssi should have 3 values: $line}")
        None
      }
    } else {
      log.warn(s"data format of AP [$apMac] error, data line should have 4 parts: $line}")
      None
    }
  }
  ...
  def line2Shoot(line:String) = {
    ...
    if(infos.length == 6){
      ...
      val src = infos(5)
      if(rssi.length == 3){
        Some(Shoot(id,apMac,clientMac,t,rssi,src))
      }else{
